...
void addCategory(Category categories[], int *countCategory) {
    ...
    do {
        ...
        } else if (strlen(newCategory.CategoryId) > 10) {
    ...
    do {
        ...
        if (strchr(newCategory.CategoryName, '\n') == NULL) {
            printf("\n\tError: Category Name must be at most 9 characters!\n"); // Ten danh muc khong duoc qua 9 ki tu
            clearCache();
            continue;
        }
...
void addProduct(Product products[], int *countProduct, Category categories[], int *countCategory)
{
    ...
    do {
        ...
        } else if (strlen(newProduct.ProductId) > 10) {
    ...
    do {
        ...
        if (strchr(newProduct.ProductName, '\n') == NULL) {
            printf("\n\tError: Product Name must be at most 9 characters!\n"); // Ten san pham khong duoc qua 10 ki tu
            clearCache();
            continue;
        }
    ...
    scanf("%9d", &newProduct.Quantity);

    // Nhap gia san pham
    printf("\n\tEnter Product Price: ");
    scanf("%9d", &newProduct.Price);
...
void editProduct(Product products[], int *countProduct, Category categories[], int *countCategory) {
    ...
    int i;
    for (i = 0; i < *countProduct; i++) {
        if (strcmp(products[i].ProductId, idProduct) == 0 && strcmp(products[i].CategoryId, IDCategory) == 0) {
            // Thong bao da tim thay
            ...
            scanf("%9d", &products[i].Quantity);
            
            // Nhap gia moi
            printf("\n\tEnter new Product Price: ");
            scanf("%9d", &products[i].Price);
...
void deleteProduct(Product products[], int *countProduct, Category categories[], int *countCategory) {
    ...
    int i, j;
    for (i = 0; i < *countProduct; i++) {
        if (strcmp(products[i].ProductId, id) == 0 && strcmp(products[i].CategoryId, IDCategory) == 0) {
            // Thong bao da tim thay
            printf("\n\tOne Product Found for ID: %s\n", id);
            printf("\tProduct informations\n");
            printf("\t--------------------------\n");
...
void mainMenu()
{
    ...
    printf("\n\t===============================================");
    printf("\n\t| [1] Login Admin.                            |\n");
    printf("\t| [2] Menu Category Management.               |\n");
    printf("\t| [3] Menu Product Management.                |\n");
...
void menuCategory(Category categories[], int *count, Product products[], int *countProduct, int *checkAdmin) {
    while (1) {
        ...
        if (*checkAdmin == 0 && (c == 2 || c == 3 || c == 4))
        {
            printf("\n\tOnly admin has permission to use.\n");
            continue;
        }
...
void menuProduct(Product products[], int *countProduct, Category categories[], int *countCategory, int *checkAdmin) {
    while (1) {
        ...
        switch (c) {
            ...
            case 3: // Sua danh muc
                editProduct(products, countProduct, categories, countCategory);
                break;

            case 4: // Xoa danh muc
                deleteProduct(products, countProduct, categories, countCategory);
                break;
            ...
            case 7: // Sap xep danh muc theo ten
                filterProductMenu(products, *countProduct, categories, *countCategory);
                break;
...
void filterProductsByCategory(Product products[], int countProduct, Category categories[], int countCategory) {
    ...
    // Kiem tra danh muc co ton tai khong
    int foundCategory = 0;
    int i;
    for (i = 0; i < countCategory; i++) {
    ...
    for (j = 0; j < countProduct; j++) {
        if (strcmp(products[j].CategoryId, categoryID) == 0 && found == 0) {
            printf("\n\t|=============|===============|=============|=============|=============|");
            printf("\n\t| %-11s | %-11s   | %-11s | %-11s | %-11s |\n", "Category ID", "Product ID", "Name", "Quantity", "Price");
            printf("\t|=============|===============|=============|=============|=============|\n");
            printf("\t| %-11s | %-11s   | %-11s | %-11d | %-11d |\n", products[j].CategoryId, products[j].ProductId, products[j].ProductName, products[j].Quantity, products[j].Price);
            printf("\t|-------------|---------------|-------------|-------------|-------------|\n");
            found = 1;
        }
        else if (strcmp(products[j].CategoryId, categoryID) == 0 && found == 1)
        {
            printf("\t| %-11s | %-11s   | %-11s | %-11d | %-11d |\n", products[j].CategoryId, products[j].ProductId, products[j].ProductName, products[j].Quantity, products[j].Price);
            printf("\t|-------------|---------------|-------------|-------------|-------------|\n");
        }
...
void filterProductsByPrice(Product products[], int countProduct) {
    ...
    int i;
    for (i = 0; i < countProduct; i++) {
        if (products[i].Price >= minPrice && products[i].Price <= maxPrice && found == 0) {
            printf("\n\t|=============|===============|=============|=============|=============|");
            printf("\n\t| %-11s | %-11s   | %-11s | %-11s | %-11s |\n", "Category ID", "Product ID", "Name", "Quantity", "Price");
            printf("\t|=============|===============|=============|=============|=============|\n");
            printf("\t| %-11s | %-11s   | %-11s | %-11d | %-11d |\n", products[i].CategoryId, products[i].ProductId, products[i].ProductName, products[i].Quantity, products[i].Price);
        ...
        else if (products[i].Price >= minPrice && products[i].Price <= maxPrice && found == 1)
        {
            printf("\t| %-11s | %-11s   | %-11s | %-11d | %-11d |\n", products[i].CategoryId, products[i].ProductId, products[i].ProductName, products[i].Quantity, products[i].Price);
            printf("\t|-------------|---------------|-------------|-------------|-------------|\n");
        }
    ...
    if (!found) {
        printf("\n\tNo items available within this price range.\n");
    }
}

// Ham lua chon loc san pham
void filterProductMenu(Product products[], int countProduct, Category categories[], int countCategory) {
    printf("\n\tEnter The Filter Choice\n");
    printf("\t1 : Category, 2 : Price\n");
    int c = choice();

    switch (c) {
        case 1: // Loc theo danh muc
            filterProductsByCategory(products, countProduct, categories, countCategory);
            goBack();
            break;

        case 2: // Loc theo gia
            filterProductsByPrice(products, countProduct);
            goBack();
            break;
